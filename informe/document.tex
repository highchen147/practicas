...
\lstset{style=myStyle}
	...
	Se puede notar en la ecuación \ref{eq:flujo_promedio} que la expresión exacta para el flujo promedio en una interfaz, $F_{i}^{n}$ () es una integral y generalmente, en consecuencia de que $u$ depende del tiempo, esta integral tampoco es trivial. Sin embargo, se sabe que la información de la onda se propaga a través del espacio con velocidad finita, lo que implica que es posible obtener $F_{i}^{n}$ a partir de los valores $u_i$ y $u_{i-1}$ \cite{LeVeque1998}:
	\begin{equation}
