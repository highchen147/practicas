...
\lstset{style=myStyle}
	...
	\subsubsection{Caso $u_L > u_R$, prob. de Riemann}
	...
	\textit{Prueba:} Sea $\phi$ una función de prueba con soporte compacto $U$ y que este último yace completamente en alguno de los dos conjuntos siguientes: $\{x<st\}$ o $\{x>st\}$. Dado que $u(x,t)$ es constante en ambos conjuntos mencionados, esta satisface la ecuación conservativa (\ref{eq:conservative_general}). Entonces, la ecuación (\ref{eq:integral_conservative1}) se satisface y consecuentemente se satisface la condición \ref{eq:weak_sol_cond} que garantiza que la solución es una solución débil.
	\begin{figure}[h]
		\centering
