...
\lstset{style=myStyle}
	...
	Si sustituimos el flujo de la ecuación de Burgers, encontramos que la solución del problema de Riemann de esta ecuación, para este caso es 
	...
	Hay múltiples soluciones débiles para este caso. Por  ejemplo, se puede demostrar que la solución \ref{eq:sol-riemann-weak-burgers} satisface este caso también, tomando en cuenta que cambiará el signo de la velocidad de propagación $s$. Sin embargo este solución no satisface la condición de entropía descrita por \cite{Cameron}.
	
	\begin{definition}
		Una discontinuidad, propagándose en el espacio $x-t$, dada por la ecuación \ref{eq:sol-riemann-weak} satisface la condición 
	\end{definition}
